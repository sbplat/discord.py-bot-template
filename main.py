...
async def on_ready():
    ...
    print(f"Bot is now online.\nLogged in as {bot.user.name}\nID: {bot.user.id}\nInvite Link: https://discord.com/oauth2/authorize?client_id={bot.user.id}&scope=bot")
...
async def on_message(message):
    if message.author.bot:
        ...
        try:
            return await message.channel.send(f"{message.author.mention}, Go away! Your blacklisted from me.")
        except:
            pass
...
async def on_guild_join(guild):
    print(f"I just joined a server!\n\nName: {guild.name}\nID: {guild.id}\nMembers: {guild.member_count}")
    await bot.change_presence(activity=discord.Game(name=f"-help| {len(bot.guilds)} servers"))

@bot.event
async def on_guild_remove(guild):
    await bot.change_presence(activity=discord.Game(name=f"cb.help | {len(bot.guilds)} servers"))
    print(f"I just left a server!\n\nName: {guild.name}\nID: {guild.id}\nMembers: {guild.member_count}")
