...
if not TOKEN:
    print("You need to put your Discord bot token in the .env file.")
    sys.exit(0)
...
async def on_message(message):
    if (message.author.bot) or (message.author.id in bot.blacklisted):
        return
...
for ext in [
    ".".join(p.parts)[: -len(".py")] for p in pathlib.Path("cogs").glob("**/*.py")
]:
